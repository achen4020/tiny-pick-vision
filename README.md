...
`target`、`build/replay` 或 `android-apk` 目标时，请先按开发指南完成标定。

Android Bench App 要求 JDK 17、Android Platform（compile SDK）34、Android
Build Tools 35.0.0、CMake 3.22.1 和 NDK r26+。Gradle 当前不固定 NDK 的
patch 版本；`26.1.10909125` 是已验证示例。原生 Make 目标兼容 NDK r24+，
两条工具链均以 Android API 24 为目标。先配置 SDK/NDK 根目录和主机标签：

```bash
export ANDROID_HOME="/path/to/Android/Sdk"
export ANDROID_NDK_HOME="$ANDROID_HOME/ndk/26.1.10909125"
# macOS: darwin-x86_64; Linux/WSL（使用 Linux SDK/NDK）: linux-x86_64;
# Windows 原生或 Git Bash: windows-x86_64
HOST_TAG="<host-tag>"
...
building the real-model `target`, `build/replay`, or `android-apk` targets.

The Android Bench App requires JDK 17, Android Platform/compile SDK 34,
Android Build Tools 35.0.0, CMake 3.22.1, and NDK r26 or newer. Gradle does
not pin an NDK patch version; `26.1.10909125` is the tested example. The native
Make targets remain compatible with NDK r24 or newer, and both toolchains
target Android API 24. Configure the SDK/NDK roots and host tag first:

```bash
export ANDROID_HOME="/path/to/Android/Sdk"
export ANDROID_NDK_HOME="$ANDROID_HOME/ndk/26.1.10909125"
# macOS: darwin-x86_64; Linux/WSL with a Linux SDK/NDK: linux-x86_64;
# native Windows or Git Bash: windows-x86_64
HOST_TAG="<host-tag>"
export PATH="$ANDROID_NDK_HOME/toolchains/llvm/prebuilt/$HOST_TAG/bin:$PATH"
make android-apk
make android-verify-sha
