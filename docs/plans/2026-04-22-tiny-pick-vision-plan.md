...
│   ├── test_hard_gates.c            # reviewer 要求的 5 条硬门槛
...
## Reviewer 要求的 5 条硬门槛（全部覆盖在 `tests/test_hard_gates.c`）
...
| HG5 | 目标工具链下 `sizeof(Blob) == 80`（编译期断言） | T0 + T9 | T0 步 4 + T9 步 3 |
...
CFLAGS_HOST    = $(CFLAGS_COMMON) -g -O0 -DTPV_HOST_BUILD
...
- [ ] **Step 1: 先写 union-find 最小测试**

```c
/* tests/test_ccl_moments.c 片段 */
#include "tpv_internal.h"
#include "testlib.h"
...
Run: `make test`。预期：此用例本身通过（只验证协议——不越界、不崩）。
...
```bash
git add Makefile scripts/check_blob_layout.sh tests/check_layout.c
git commit -m "build(t9): cross-compile + 20KB size gate + HG5 layout assert"
```
...
- Create: `tests/test_hard_gates.c`（汇总 HG1..HG5 入口）
...
/* 命令行：replay <frames_dir> > out.csv
   ...
   输出 CSV：frame_id,status,class_id,x,y,theta_x10,confidence
...
```bash
git add tests/test_property.c tests/test_hard_gates.c tools/replay.c
