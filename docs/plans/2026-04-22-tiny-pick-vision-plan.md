...
- [ ] **Step 7b: 在 `tests/test_ccl_moments.c` 末尾追加 main()**
...
> **测试文件统一约定**：每个 `tests/test_*.c` **必须**自带 `main()`，
> 在末尾 `RUN(...)` 列出本文件的所有 `TEST(...)`，最后 `FINISH()`。
> 顶层 Makefile 把每个文件单独 link 成 `build/test_<name>` 可执行；缺
> `main()` 会立即出现链接失败。后续每个 task 的最后一个 step 都按这个
> 模板补 main()。
...
并在 `tests/test_shape_features.c` 末尾加：
...
CAL_LIB_SRCS = stats.c separability.c codegen.c frame_io.c
